...
module.exports = {
  ...
  getPhrases: function (req, res, next){
    ...
    if(_.hasIn(p, 'limit')){
     ...
     Phrase.combineLanguages(_.extend(p, {country_language_id: sett.country.language.id,
                                        language_id: sett.language.id, limit: limit}), function(err, phrases){
  ...
  getJwtPhrases: function (req, res, next){
    try{
      ...
      Phrase.combineLanguages(_.extend(p, {country_language_id: sett.country.language.id, language_id: sett.language.id, limit: limit}), function(err, phrases){
  ...
  addJwtPhrase: function (req, res, next){
    var p = req.params.all();
    var sett = req.session.setting;

    Phrase.addPhrase(_.extend(p, {
      country_language_id: sett.country.language.id,
      language_id: sett.language.id,
      phrase_native_flag_prefix: sett.country.language.prefix,
      phrase_language_flag_prefix: sett.language.prefix
    }), function(err, phrases){
      if(err) return res.negotiate(err);
