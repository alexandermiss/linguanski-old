...
module.exports = {
  ...
  async listPost (opts, cb){
    ...
    // Post.find({}).populate('native').populate('learning').skip(skip).limit(parseInt(opts.limit)).sort('createdAt DESC').meta({fetch: true}).exec(function(err, posts){
    //
    //   if(err) {
